...
class HlsJsPlayer extends Player {
  constructor (options) {
    ...
    Object.defineProperty(player, 'src', {
      ...
      set (url) {
        util.removeClass(player.root, 'royplayer-is-live')
        const liveDom = document.querySelector('.royplayer-live')
        if (liveDom) {
    ...
    this.once('complete', () => {
      ...
      if(player.config.isLive) {
        Player.util.addClass(player.root, 'royplayer-is-live')
        const live = Player.util.createDom('xg-live', '正在直播', {}, 'royplayer-live')
    ...
    hls.on(Hls.Events.LEVEL_LOADED, (name, e) => {
      if (!hls.inited) {
        ...
        if (e && e.details && e.details.live) {
          util.addClass(player.root, 'royplayer-is-live')
          const live = util.createDom('xg-live', '正在直播', {}, 'royplayer-live')
          player.controls.appendChild(live)
        }
      }
    })
