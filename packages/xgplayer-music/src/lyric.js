...
class Lyric {
  ...
  show () {
    ...
    if (dom && dom.nodeType === 1) {
      const lrcWrap = Player.util.createDom('div', `<div></div>`, {}, 'royplayer-lrcWrap')
      dom.appendChild(lrcWrap)
      this.list.forEach(item => {
        lyrbicTxts.push(`<xg-lyric-item class="royplayer-lyric-item" data-idx="${item.idx}">${item.lyric.replace(/[\r\n]/g, '')}</xg-lyric-item>`)
      })
      lrcWrap.innerHTML = lyrbicTxts.join('')
      const lrcForward = Player.util.createDom('xg-lrcForward', `<div></div>`, {}, 'royplayer-lrcForward')
      ...
      const lrcBack = Player.util.createDom('xg-lrcBack', `<div></div>`, {}, 'royplayer-lrcBack')
      ...
      this.__updateHandle__ = (item) => {
        let domWrap = this.dom.querySelector('.royplayer-lrcWrap')
        let activeDom = domWrap.querySelector('.royplayer-lyric-item-active')
        let offsetHeight = domWrap.offsetHeight
        let activeTop
        if (activeDom) {
          activeDom.className = 'royplayer-lyric-item'
        }
        activeDom = domWrap.querySelector(`.royplayer-lyric-item[data-idx="${item.idx}"]`)
        if (activeDom) {
          activeDom.className = 'royplayer-lyric-item royplayer-lyric-item-active'
