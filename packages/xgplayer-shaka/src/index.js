...
class ShakaJsPlayer extends Player {
  constructor (options) {
    ...
    if (Shaka.Player.isBrowserSupported()) {
      ...
      if (this.player_.isLive()) {
        this.util.addClass(this.player.root, 'royplayer-is-live')
        let live = this.util.createDom('xg-live', '正在直播', {}, 'royplayer-live')
  ...
  updateDefinition () {
    let self = this
    let ul = this.util.createDom('xg-definition', '', {tabindex: 3}, 'royplayer-definition')
    ...
    if (this.content instanceof Array && this.content.length > 1) {
      ...
      // });
      this.util.addClass(this.player.root, 'royplayer-is-definition')
      ...
      let urlInRoot = root.querySelector('.royplayer-definition')
      if (urlInRoot) {
        urlInRoot.innerHTML = '<xg-tips class="royplayer-tips">清晰度</xg-tips>' + tmp.join('')
      } else {
        ul.innerHTML = '<xg-tips class="royplayer-tips">清晰度</xg-tips>' + tmp.join('')
        root.appendChild(ul)
      }
    }
    ...
    ['touchstart', 'click'].forEach(item => {
      ul.addEventListener(item, function (ev) {
        ...
        if (li && li.tagName.toLocaleLowerCase() === 'li') {
          ...
          self.util.addClass(ul, 'royplayer-definition-active')
          ul.focus()
        }
      }, false)
    })
    ul.addEventListener('blur', (e) => {
      e.preventDefault()
      e.stopPropagation()
      self.util.removeClass(ul, 'royplayer-definition-active')
    })
