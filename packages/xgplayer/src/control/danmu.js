...
let danmu = function () {
  ...
  if (player.config.danmu) {
    let container = Player.util.createDom('xg-bullet', '', {}, 'royplayer-bullet')
    ...
    player.once('complete', () => {
      ...
      Player.util.addClass(container, 'royplayer-has-bullet')
        ...
        ['click', 'touchend'].forEach(item => {
          player.bulletBtn.addEventListener(item, function (e) {
            ...
            if (Player.util.hasClass(player.bulletBtn, 'danmu-switch-active')) {
              ...
              Player.util.removeClass(container, 'royplayer-has-bullet')
              danmujs.stop()
            }
          }, false)
        })
