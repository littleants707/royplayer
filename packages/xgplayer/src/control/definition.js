...
let definition = function () {
  ...
  let ul = util.createDom('xg-definition', '', {tabindex: 3}, 'royplayer-definition'), root = player.controls
    ...
    if (urlInRoot) {
      ...
      if (!player.config.definitionActive || player.config.definitionActive === 'hover') {
        cur.addEventListener('mouseenter', (e) => {
          e.preventDefault()
          e.stopPropagation()
          util.addClass(player.root, 'royplayer-definition-active')
          urlInRoot.focus()
        })
      }
    } else {
  ...
  function resourceReadyFunc (listArr) {
    ...
    if (list && list instanceof Array && list.length > 1) {
      util.addClass(player.root, 'royplayer-is-definition')
  ...
  ['touchend', 'click'].forEach(item => {
    ul.addEventListener(item, function (e) {
      ...
      if (li && li.tagName.toLocaleLowerCase() === 'li') {
        ...
        if (sniffer.device === 'mobile') {
          util.removeClass(player.root, 'royplayer-definition-active')
        }
