...
let fullscreen = function () {
  ...
  let btn = util.createDom('xg-fullscreen', `<xg-icon class="royplayer-icon"><svg xmlns="http://www.w3.org/2000/svg" width="1024" height="1024" viewBox="0 0 1024 1024">
            <path transform="scale(${scale} ${scale})" d="${iconPath.default}"></path>
        </svg></xg-icon>`, {}, 'royplayer-fullscreen')
  ...
  let tips = util.createDom('xg-tips', tipsFull, {}, 'royplayer-tips')
  ...
  let getFullscreen = function (el) {
    ...
    if (el.requestFullscreen) {
      ...
      util.addClass(el, 'royplayer-fullscreen-active')
    }
  }
  let exitFullscreen = function (el) {
    ...
    util.removeClass(el, 'royplayer-cssfullscreen-active')
    path.setAttribute('d', iconPath.default)
    ...
    if (document.exitFullscreen) {
      ...
      util.removeClass(el, 'royplayer-fullscreen-active')
    }
  }
  ...
  ['click', 'touchend'].forEach(item => {
    btn.addEventListener(item, function (e) {
      e.preventDefault()
      e.stopPropagation()
      if (util.hasClass(container, 'royplayer-fullscreen-active') || util.hasClass(container, 'royplayer-is-fullscreen')) {
  ...
  fullsrceenChangeEv.forEach(item => {
    document.addEventListener(item, function (e) {
      ...
      if (!document.fullscreenElement &&!document.webkitIsFullScreen &&!document.mozFullScreen &&!document.msFullscreenElement) { 
        ...
        util.removeClass(container, 'royplayer-cssfullscreen-active')
        path.setAttribute('d', iconPath.default)
        tips.textContent = tipsFull
        util.removeClass(container, 'royplayer-fullscreen-active')
      }
    })
  })
  ...
  let handle = function (e) {
    ...
    if (fullscreenEl && fullscreenEl === container) {
      util.addClass(container, 'royplayer-is-fullscreen')
      path.setAttribute('d', iconPath.active)
      tips.textContent = tipsExitFull
      player.emit('requestFullscreen')
    } else {
      util.removeClass(container, 'royplayer-is-fullscreen')
      path.setAttribute('d', iconPath.default)
      tips.textContent = tipsFull
      player.emit('exitFullscreen')
    }
