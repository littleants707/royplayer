...
let pc = function () {
  ...
  let enter = util.createDom('xg-enter', '<xg-enter-logo class="royplayer-enter-logo"></xg-enter-logo><xg-enter-tips class="royplayer-enter-tips"></xg-player-tips>', {}, 'royplayer-enter')
  let logo = enter.querySelector('.royplayer-enter-logo')
  root.appendChild(btn)
  root.appendChild(enter)
  let enterTips = enter.querySelector('.royplayer-enter-tips')
  ...
  function startClcCanplay () {
    util.removeClass(root, 'royplayer-is-enter')
  }
  ...
  function startClc (e) {
    ...
    //   return
    // }
    if (util.hasClass(root, 'royplayer-nostart')) {
      util.removeClass(root, 'royplayer-nostart') // for ie quick switch
      util.addClass(root, 'royplayer-is-enter')
      ...
      if (player.paused) {
        util.removeClass(root, 'royplayer-nostart royplayer-isloading')
        setTimeout(() => {
          player.play()
        }, 10)
      }
  ...
  function startAniEnd (e) {
    e.preventDefault()
    util.removeClass(btn, 'royplayer-start-interact')
  ...
  function playFunc () {
    ...
    util.addClass(btn, 'royplayer-start-interact')
  }
    ...
    if (!player.config.closeVideoClick) {
      ...
      if (clk === 1) {
        _click_ = setTimeout(function () {
          if (util.hasClass(player.root, 'royplayer-nostart')) {
            return false
  ...
  player.onElementDblclick = function (e, element) {
    ...
    if (!player.config.closeVideoDblclick) {
      let fullscreen = controls.querySelector('.royplayer-fullscreen')
      if (fullscreen) {
        let clk
