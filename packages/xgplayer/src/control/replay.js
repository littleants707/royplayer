...
let replay = function () {
  ...
  if (centerBtn.type === 'img') {
    btn = util.createDom('xg-replay', '<div class="royplayer-replay-img"></div>', {}, 'royplayer-replay')
    img = btn.querySelector('.royplayer-replay-img')
    ...
    btn = util.createDom('xg-replay', `
          <svg class="royplayer-replay-svg" xmlns="http://www.w3.org/2000/svg" width="78" height="78" viewbox="0 0 78 78">
            <path d="${iconPath.replay}"></path>
          </svg>
          <xg-replay-txt class="royplayer-replay-txt">重播</xg-replay-txt>
          `, {}, 'royplayer-replay')
    svg = btn.querySelector('.royplayer-replay-svg')
  }
  ...
  function endedFunc () {
    if (centerBtn.type === 'img') {
      ...
      btn.querySelector('.royplayer-replay-txt').textContent = player.lang.REPLAY
