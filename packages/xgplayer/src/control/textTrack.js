...
let textTrack = function () {
  ...
  let ul = util.createDom('xg-textTrack', '', {tabindex: 7}, 'royplayer-textTrack'), root = player.controls
  let list = player.config.textTrack
  if (list && Array.isArray(list) && list.length > 1) {
    util.addClass(player.root, 'royplayer-is-textTrack')
    player.on('canplay', function () {
      ...
      let urlInRoot = root.querySelector('.royplayer-textTrack')
      if (urlInRoot) {
        ...
        cur.addEventListener('mouseenter', (e) => {
          e.preventDefault()
          e.stopPropagation()
          util.addClass(player.root, 'royplayer-textTrack-active')
          urlInRoot.focus()
        })
      } else {
    ...
    ul.addEventListener(item, function (e) {
      ...
      if (li && li.tagName.toLocaleLowerCase() === 'li') {
        ...
        if (li.innerHTML === '关闭') {
          ...
          util.removeClass(player.root, 'royplayer-textTrack-active')
        } else {
          if (!util.hasClass(player.root, 'royplayer-textTrack-active')) {
            util.addClass(player.root, 'royplayer-textTrack-active')
          }
