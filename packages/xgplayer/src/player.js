...
class Player extends Proxy {
  constructor (options) {
    ...
    util.addClass(this.root, `royplayer royplayer-${sniffer.device} royplayer-nostart ${this.config.controls ? '' : 'no-controls'}`)
    ...
    if (this.config.videoInit) {
      if(util.hasClass(this.root, 'royplayer-nostart')) {
        this.start()
      }
    }
  }
    ...
    this.canPlayFunc = function () {
      ...
      if (playPromise !== undefined && playPromise) {
        playPromise.then(function () {
          ...
          Player.util.addClass(player.root, 'royplayer-is-autoplay')
        })
      }
  ...
  replay () {
    ...
    // ie9 bugfix
    util.removeClass(this.root, 'royplayer-ended')
    this.logParams = {
      bc: 0,
      bu_acu_t: 0,
      played: [],
  ...
  getPIP () {
    ...
    let Top = ro.top
    let Left = ro.left
    let dragLay = util.createDom('xg-pip-lay', '<div></div>', {}, 'royplayer-pip-lay')
    this.root.appendChild(dragLay)
    let dragHandle = util.createDom('xg-pip-drag', '<div class="drag-handle"><span>点击按住可拖动视频</span></div>', {tabindex: 9}, 'royplayer-pip-drag')
    this.root.appendChild(dragHandle)
    let draggie = new Draggabilly('.royplayer', {
      handle: '.drag-handle'
    })
    util.addClass(this.root, 'royplayer-pip-active')
  ...
  exitPIP () {
    util.removeClass(this.root, 'royplayer-pip-active')
  ...
  onFocus () {
    let player = this
    util.removeClass(this.root, 'royplayer-inactive')
    if (player.userTimer) {
  ...
  onBlur () {
    ...
    if (!this.paused && !this.ended) {
      util.addClass(this.root, 'royplayer-inactive')
    }
  }

  onPlay () {
    util.addClass(this.root, 'royplayer-playing')
    util.removeClass(this.root, 'royplayer-pause')
  }

  onPause () {
    util.addClass(this.root, 'royplayer-pause')
    if (this.userTimer) {
  ...
  onEnded () {
    util.addClass(this.root, 'royplayer-ended')
    util.removeClass(this.root, 'royplayer-playing')
  }
  ...
  onSeeked () {
    ...
    util.removeClass(this.root, 'royplayer-isloading')
  }
  ...
  onWaiting () {
    ...
    self.waitTimer = setTimeout(function () {
      util.addClass(self.root, 'royplayer-isloading')
    }, 500)
  }
  ...
  onPlaying () {
    ...
    util.removeClass(this.root, 'royplayer-isloading royplayer-nostart royplayer-pause royplayer-ended royplayer-is-error royplayer-replay')
    util.addClass(this.root, 'royplayer-playing')
  }
  ...
  onKeydown (event, player) {
    ...
    if (e && (e.keyCode === 40 || e.keyCode === 38)) {
      if (player.controls) {
        let volumeSlider = player.controls.querySelector('.royplayer-slider')
        if (volumeSlider) {
          if (util.hasClass(volumeSlider, 'royplayer-none')) {
            util.removeClass(volumeSlider, 'royplayer-none')
          }
          if (player.sliderTimer) {
            clearTimeout(player.sliderTimer)
          }
          player.sliderTimer = setTimeout(function () {
            util.addClass(volumeSlider, 'royplayer-none')
          }, player.config.inactive)
        }
      }
